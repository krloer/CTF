...
class MathKt__MathJVMKt extends MathKt__MathHKt {
    ...
    public static final double asinh(double x) {
        if (x >= Constants.taylor_n_bound) {
            if (x > Constants.upper_taylor_n_bound) {
                ...
                Double.isNaN(d);
                Double.isNaN(d2);
                return Math.log((x * d) + (d2 / (d * x)));
            }
            double d3 = 1;
            Double.isNaN(d3);
            return Math.log(Math.sqrt((x * x) + d3) + x);
    ...
    public static final int roundToInt(double $this$roundToInt) {
        ...
        if ($this$roundToInt > ((double) IntCompanionObject.MAX_VALUE)) {
            return IntCompanionObject.MAX_VALUE;
        }
        return $this$roundToInt < ((double) IntCompanionObject.MIN_VALUE) ? IntCompanionObject.MIN_VALUE : (int) Math.round($this$roundToInt);
    }
