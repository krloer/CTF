...
def encrypt_stage_one(message, key):
    u = [s for s in sorted(zip(key, range(len(key))))] # number paired with index, sorted by size of number
    res = ''
...
while len(rand_nums) != 8:
    tmp = int.from_bytes(os.urandom(1), "big") # 8 numbers
    if tmp not in rand_nums:
        rand_nums.append(tmp)

for _ in range(42):
    # Answer to the Ultimate Question of Life, the Universe, and Everything...
    flag = encrypt_stage_one(flag, rand_nums) #randomizes order of letters
