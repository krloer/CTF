...
public class mysteryMethods{
    ...
    public static String encryptInput(String input) {
        String flag = input;
        flag = unknown2(flag, 345345345); // b64 encode
        flag = unknown1(flag); // reverses
        flag = unknown2(flag, 00000); // b64 encode
        flag = unknown(flag, 25); // ascii minus two
        return flag;
    }
    ...
    public static String unknown(String input, int something) {
        ...
        for (char c : input.toCharArray()) {
            if (Character.isLetter(c)) {
                ...
                int offset = (c - base + something) % 26; //e: 69 - 65 + 25 = 29 % 26 = 3
                if (offset < 0) {
                    offset += 26;
                }
                c = (char) (base + offset);
